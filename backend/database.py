...
def get_avg_run_time(num_segments: int = 5) -> Optional[float]:
    """Calculate average run time from the last N completed segments.

    Args:
        num_segments: Number of recent completed segments to average (default 5)

    Returns:
        Average run time in seconds, or None if no completed segments
    """
    with get_connection() as conn:
        cursor = conn.cursor()
        # Get the average of the last N completed segments
        cursor.execute("""
...
def _get_lora_base_and_type(filename: str) -> tuple:
    ...
    high_patterns = [
        ...
        r'[a-z]High[_.-]',  # TurnsHigh- (no separator before, separator after)
        r' high ',  # space before and after (wlkng high 250909a)
        r'-H-',
    ...
    low_patterns = [
        ...
        r'[a-z]Low[_.-]',  # TurnsLow- (no separator before, separator after)
        r' low ',  # space before and after (wlkng low 250909a)
        r'-L-',
